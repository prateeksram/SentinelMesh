"""Telemetry aggregation (handoff ┬º6, D3; handoff-2 P2).

Self-reported duty cycle is the primary signal: each workload measures its own
busy time and reports busy_pct per unit. `metric` is opaque ΓÇö stored and
forwarded without interpretation, so a new unit type never requires a server
change.

Units can have multiple *sources* (the laptop NPU is claimed by both the
GenieX desk and the FX depth session). Reports carry an optional "source"
key; the snapshot merges live sources per unit: busy = max, metrics merged,
state lines joined, freshest age wins. A unit is stale only when ALL its
sources are stale ΓÇö dead Γëá idle, and the dashboard must show which.
"""
        ...
        for key, entry in self._devices.items():
            ...
            for unit, sources in entry["units"].items():
                ...
                for rec in sources.values():
                    ...
                    if age > STALE_MS:
                        continue          # stale sources don't pollute live ones
                    try:
                ...
                if best_age is None:      # ALL sources stale ΓÇö surface the freshest
