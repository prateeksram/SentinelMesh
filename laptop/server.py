    ...
    async def run_match(self):
        ...
        try:
            for _ in range(KICKS):
                ...
                # SceneEngine shootWindow must never undercut the play wait.
...
async def scene_upload(request):
    ...
    if result.get("ok") and result.get("scene"):
        ...
        game.k_window = max(float(d.get("shootWindow") or 0), float(MIN_SHOOT_WINDOW), float(SHOOT_WINDOW))
        game.k_beat = d["powerBeat"]
        game.scene = scene
